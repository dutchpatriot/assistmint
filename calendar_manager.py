...
def parse_time(time_str, silent=False):
    ...
    try:
        ...
        if len(time_parts) == 2:  # e.g., "seven am", "five pm"
            ...
            if not silent:
                speak(f"Sorry, I couldn't understand the time {time_str}.")
            return None
        ...
        if period not in ["am", "pm"]:
            if not silent:
                speak(f"Please specify AM or PM.")
            return None
...
def parse_date(date_str, silent=False, lang=None):
    ...
    if parsed_date is None:
        if not silent:
            speak(f"Sorry, I couldn't understand the date {date_str}.")
        return None
...
def add_event_to_calendar(event_name, start_time, end_time, date="today"):
    ...
    if CALENDAR_BACKEND == "evolution":
        if not EVOLUTION_AVAILABLE:
            speak("Evolution calendar is not available. Please install gir1.2-ecal-2.0.")
            return
    ...
    # Use Google Calendar if configured and available
        ...
        if not GCALCLI_AVAILABLE:
            speak("Google Calendar is not set up. Please install gcalcli first.")
...
def _add_event_evolution_extended(event_name, event_date, start_time, end_time,
    ...
    if client is None:
        speak("Could not connect to Evolution calendar.")
        return
    ...
    try:
        ...
        if success:
            ...
            speak(f"Added {event_name} to your calendar.")
        else:
        ...
        speak("Sorry, there was an error adding the event.")
...
def add_event_to_calendar_extended(event_name, start_time, end_time, date="today",
    ...
    if CALENDAR_BACKEND == "evolution":
        if not EVOLUTION_AVAILABLE:
            speak("Evolution calendar is not available.")
            return
...
def _add_event_google(event_name, event_date, start_time, end_time):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            speak(f"Added {event_name} to your Google Calendar.")
        else:
            print(f"[GOOGLE] Error: {result.stderr}")
            speak("Sorry, I couldn't add the event to Google Calendar.")

    except subprocess.TimeoutExpired:
        speak("Google Calendar timed out.")
    except Exception as e:
        print(f"[GOOGLE] Exception: {e}")
        speak("Sorry, there was an error adding to Google Calendar.")
...
def _add_event_local(event_name, event_date, start_time, end_time):
    ...
    speak(f"The event {event_name} has been added to your calendar.")
    ...
    try:
        ...
        if success and events:
            ...
            if event_list:
                ...
                if week:
                    speak("You have no events this week.")
                else:
                    speak(f"You have no events on {start_date.strftime('%A, %B %d')}.")
        else:
            if week:
                speak("You have no events this week.")
            else:
...
def _check_calendar_google(date="today", week=False, specific_week_start=None):
    ...
    try:
        ...
        speak("Sorry, I couldn't check your Google Calendar.")
...
def remove_event(event_name, date):
    ...
    # Use Google Calendar if configured
        ...
        speak("Removing events from Google Calendar is not yet supported.")
        return
    else:
...
def _remove_event_evolution(event_name, event_date):
    ...
    try:
        ...
        if not success or not events:
            speak(f"No events found on {check_date.strftime('%A, %B %d')}.")
            return
        ...
        if not found_uid:
            speak(f"No event matching '{event_name}' found on {check_date.strftime('%A, %B %d')}.")
            return
        ...
        if success:
            ...
            speak(f"Removed '{found_event}' from your calendar.")
        else:
            speak("Sorry, I couldn't remove the event.")

    except Exception as e:
        print(f"[EVOLUTION] Error removing event: {e}")
        speak("Sorry, there was an error removing the event.")
...
def _remove_event_local(event_name, event_date):
    ...
    try:
        ...
        if removed:
            speak(f"The event {event_name} on {event_date_str} has been removed from your calendar.")
        else:
            speak(f"No matching event found for {event_name} on {event_date_str}.")
...
def clear_calendar(date=None, week=False):
    ...
    # Use Google Calendar if configured
        ...
        speak("Clearing events from Google Calendar is not yet supported.")
        return
    else:
...
def _clear_calendar_evolution(start_date, end_date, week=False):
    ...
    try:
        ...
        if not success or not events:
            if week:
                speak(f"No events found for the week.")
            else:
                speak(f"No events found on {start_date.strftime('%A, %B %d')}.")
            return
        ...
        if week:
            ...
            speak(f"Cleared {deleted_count} events for the week.")
        else:
            print(f"[EVOLUTION] Cleared {deleted_count} events on {start_date}")
            speak(f"Cleared {deleted_count} events on {start_date.strftime('%A, %B %d')}.")

    except Exception as e:
        print(f"[EVOLUTION] Error clearing calendar: {e}")
        speak("Sorry, there was an error clearing the calendar.")
...
def _clear_calendar_local(start_date, end_date, week=False):
    ...
    try:
        ...
        if week:
            week_str = f"{start_date.strftime('%A, %B %d')} through {end_date.strftime('%A, %B %d')}"
            speak(f"All events for the week of {week_str} have been cleared from your calendar.")
        else:
            speak(f"All events on {start_date.strftime('%A, %B %d')} have been cleared from your calendar.")
