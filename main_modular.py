...
Usage:
    python main_modular.py [--model MODEL] [--device N] [--voice] [--type]
...
# Core imports
from core.logger import cmd, set_use_emojis
from core.resources import get_resource_manager
from core.modules import get_module_loader, ModuleContext
...
from core.nlp import apply_corrections, is_hallucination, get_intent_router
from core.actions import execute_action, is_system_action
...
def init_assistant():
    ...
    try:
        ...
        from core.audio.stt import get_stt_engine
        from core.audio.tts import get_tts_engine
        from core.resources.manager import ResourceType
...
def process_command(text: str, context: ModuleContext, loader) -> bool:
    ...
    try:
        ...
        from core.audio.tts import set_language
...
""")
    if not _quiet_help:
        speak("Calendar: add to calendar, check my agenda. Session: clear session. Dictation: say dictate to type. Or just ask me anything.")
